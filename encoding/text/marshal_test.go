...
func TestEncode(t *testing.T) {
	tests := []struct {
		...
		{0xc0b634e19e5a9a4e, `(key = "42", value = (int32 = -123))`, 0},
		{0x967c8fe21790b0fb, `(key = "float", value = (float64 = 3.14))`, 0},
		{0xdf35cb2e1f5ea087, `(key = "bool", value = (bool = false))`, 0},
		{0xb167974479102805, `(map = [(key = "foo", value = (void = void)), (key = "bar", value = (void = void))])`, 0},
		{0x81fdbfdc91779421, `(map = [])`, 0},
		{0x8e85252144f61858, `(data = "Hi\xde\xad\xbe\xef\xca\xfe")`, 0},
		{0xc21398a8474837ba, `(voidList = [void, void])`, 0},
		{0xde82c2eeb3a4b07c, `(boolList = [true, false, true, false])`, 0},
		{0xf9e3ffc179272aa2, `(int8List = [1, -2, 3])`, 0},
		{0xfc421b96ec6ad2b6, `(int64List = [1, -2, 3])`, 0},
		{0xb3034b89d02775a5, `(uint8List = [255, 0, 1])`, 0},
		{0x9246c307e46ad03b, `(uint64List = [1, 2, 3])`, 0},
		{0xd012128a1a9cb7fc, `(float32List = [0.5, 3.14, -2])`, 0},
		{0xf16c386c66d492e2, `(textList = ["foo", "bar", "baz"])`, 0},
		{0xe14f4d42aa55de8c, `(dataList = ["\xde\xad\xbe\xef", "\xca\xfe"])`, 0},
		{0xe88c91698f7f0b73, `(cheese = gouda)`, 0},
		{0x9c51b843b337490b, `(cheeseList = [gouda, cheddar])`, 0},
		{0x81e2aadb8bfb237b, `(matrix = [[1, 2, 3], [4, 5, 6]])`, 0},
		{0xde82c2eeb3a4b07c, `(
    boolList = [
        true,
        false,
...
func TestEncodeList(t *testing.T) {
	tests := []struct {
		...
		{0x90c9e81e6418df8e, `[(key = "foo", value = (void = void)), (key = "bar", value = (void = void))]`, 0},
		{0x90c9e81e6418df8e, `[
    (
        key = "foo",
        value = (
